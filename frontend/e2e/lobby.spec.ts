...
test.describe('Lobby Page', () => {
  ...
  test('should display game title', async ({ page }) => {
    await expect(page.getByRole('heading', { level: 1 })).toContainText('Emoji Guesser');
  });

  test('should show connection status', async ({ page }) => {
    // Connection status should be visible
    await expect(page.getByText(/Status:/)).toBeVisible();
    // Wait for either connected or disconnected state
  ...
  test('should have game creation options', async ({ page }) => {
    ...
    await expect(page.getByLabel('Private Game')).toBeVisible();
    await expect(page.getByLabel('Public Game')).toBeVisible();
  });
  ...
  test('should toggle between public and private game options', async ({ page }) => {
    const privateRadio = page.getByLabel('Private Game');
    const publicRadio = page.getByLabel('Public Game');

    // Initially private should be selected
  ...
  test('should display public games section', async ({ page }) => {
    await expect(page.getByRole('heading', { name: /Public Games/i })).toBeVisible();
  });
...
test.describe('Accessibility', () => {
  test('should have proper heading structure', async ({ page }) => {
    ...
    await expect(h1).toContainText('Emoji Guesser');
  });

  test('form inputs should have proper labels', async ({ page }) => {
    await page.goto('/');

    // Check that radio buttons have accessible labels
    await expect(page.getByRole('radio', { name: /Private Game/i })).toBeVisible();
    await expect(page.getByRole('radio', { name: /Public Game/i })).toBeVisible();
  });
});
